...
parameter_set = {'Na':N, 'Nb':0.1*N, 'gammaI':gammaI, 'R0a':R0, 'R0b':R0}
...
if np.mean(pivoted.values)>0:
    im = ax.pcolormesh(pivoted.columns, pivoted.index, pivoted, vmin = 0, vmax = 4, cmap = 'cividis')
else:
    im = ax.pcolormesh(pivoted.columns, pivoted.index, pivoted, vmin = -4, vmax = 0, cmap = 'cividis')
...
plt.tight_layout()
plt.savefig('../figures/figsi_mathematical_model_different_pop_size.png', dpi = 300)
plt.savefig('../figures/figsi_mathematical_model_different_pop_size.pdf')