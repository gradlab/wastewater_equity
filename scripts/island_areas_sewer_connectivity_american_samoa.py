...
# demographic data
census_dp1 = pd.read_csv('../data/islands/american_samoa/DECENNIALDPAS2020.DP1-Data.csv')
census_dp1 = census_dp1[2:]
census_dp1.reset_index(inplace = True, drop = True)

# economic data (at household level)
census_dp3 = pd.read_csv('../data/islands/american_samoa/DECENNIALDPAS2020.DP3-Data.csv')
census_dp3 = census_dp3[2:]
census_dp3.reset_index(inplace = True, drop = True)

# household size and composition data
census_dp4 = pd.read_csv('../data/islands/american_samoa/DECENNIALDPAS2020.DP4-Data.csv')
census_dp4 = census_dp4[2:]
...
for geo_id in geo_ids: 
    county_subdivision_fips.append(int(geo_id[geo_id.find('US60')+4:]))
census_island['place_fips'] = county_subdivision_fips

census_island['island'] = 'American Samoa'
...
merged['frac_other'] = census['DP4_0066C']/census['DP4_0063C']

print('Overall fraction of households on sewer: ' + str(census['DP4_0064C'].sum()/census['DP4_0063C'].sum()))
...
merged.to_csv('../outputs/island_areas_sewer_connectivity_american_samoa.csv')