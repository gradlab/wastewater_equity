...
# demographic data
census_dp1 = pd.read_csv('../data/islands/northern_mariana_islands/DECENNIALDPMP2020.DP1-Data.csv')
census_dp1 = census_dp1[1:]
census_dp1.reset_index(inplace = True, drop = True)

# economic data (at household level)
census_dp3 = pd.read_csv('../data/islands/northern_mariana_islands/DECENNIALDPMP2020.DP3-Data.csv')
census_dp3 = census_dp3[1:]
census_dp3.reset_index(inplace = True, drop = True)

# household size and composition data
census_dp4 = pd.read_csv('../data/islands/northern_mariana_islands/DECENNIALDPMP2020.DP4-Data.csv')
...
for geo_id in geo_ids: 
    county_subdivision_fips.append(int(geo_id[geo_id.find('US69')+4:]))
census_island['place_fips'] = county_subdivision_fips

census_island['island'] = 'N. Mariana Is.'
...
merged.drop(['GEO_ID', 'NAME', 'Unnamed: 586'], inplace = True, axis = 'columns')

# Threshold on CDPs with at least 20 population and 5 households
total_pop_var = 'DP1_0001C'
total_households_var = 'DP1_0131C'
pop_thresh = 20
households_thresh = 5
...
merged['frac_sewer'] = census['DP4_0063C']/census['DP4_0062C']
merged['frac_septic'] = census['DP4_0064C']/census['DP4_0062C']
merged['frac_other'] = census['DP4_0065C']/census['DP4_0062C']

# Print summary statistics
print('Overall fraction of households on sewer: ' + str(census['DP4_0063C'].sum()/census['DP4_0062C'].sum()))
...
merged.to_csv('../outputs/island_areas_sewer_connectivity_northern_mariana_islands.csv')