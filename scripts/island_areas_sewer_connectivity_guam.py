...
# demographic data
census_dp1 = pd.read_csv('../data/islands/guam/DECENNIALDPGU2020.DP1-Data.csv')
census_dp1 = census_dp1[1:]
census_dp1.reset_index(inplace = True, drop = True)

# economic data (at household level)
census_dp3 = pd.read_csv('../data/islands/guam/DECENNIALDPGU2020.DP3-Data.csv')
census_dp3 = census_dp3[1:]
census_dp3.reset_index(inplace = True, drop = True)

# household size and composition data
census_dp4 = pd.read_csv('../data/islands/guam/DECENNIALDPGU2020.DP4-Data.csv')
...
for geo_id in geo_ids: 
    county_subdivision_fips.append(int(geo_id[geo_id.find('US66')+4:]))
census_island['place_fips'] = county_subdivision_fips

census_island['island'] = 'Guam'
...
shapes.loc[shapes['NAMELSAD']=='HagÃ¥tÃ±a CDP', 'NAMELSAD'] = 'Hagåtña CDP'
...
total_households_var = 'DP1_0135C'
pop_thresh = 20
households_thresh = 5
...
merged.to_csv('../outputs/island_areas_sewer_connectivity_guam.csv')