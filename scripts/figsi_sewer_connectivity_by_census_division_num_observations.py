...
# Make plot
fig, ax = plt.subplots(6,1, figsize = (8.5,11), sharex = True, gridspec_kw = {'wspace':0, 'hspace':0.5})
...
# Reformat the descriptions to make them easier to display
...
for variable in variables:
    ...
    for k in x:
        ...
        if variable_type=='overall':
            ...
            ax[i].text(k*8, y+yerr_upper-10, row['num_observations'].values[0], horizontalalignment='center', fontsize = 5)
        ...
        # Demographic and economic variables
            ...
            for variable_value in variable_values:
                ...
                ax[i].text(k*8-round(len(variable_values)/2)+j*width, dely+yerr_upper+1, row['num_observations'].values[0], horizontalalignment='center', fontsize = 5)
                ax[i].plot([k*8-4, k*8-4], [-40, 40], linestyle = ':', color = 'k', linewidth = 0.5, alpha = 0.5)
...
fig.suptitle('By census division (2021)')
plt.tight_layout()
plt.savefig('../figures/figsi_sewer_connectivity_by_census_division_num_observations.png', dpi = 300)
plt.savefig('../figures/figsi_sewer_connectivity_by_census_division_num_observations.pdf', dpi = 300)